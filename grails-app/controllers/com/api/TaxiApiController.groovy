...
class TaxiApiController {
	...
	def getMsg = {
		...
		log.error "taxista obteniendo mensaje"
		log.error params as JSON
		if(tok){
			log.error tok.username
			log.error params?.lng
				...
				if(params?.id){
					...
					if(oper){
						...
						if(onl){
							...
							onl.save(flush:true)
							log.error "Finalizando el viaje"
