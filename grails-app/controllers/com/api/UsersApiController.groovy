...
class UsersApiController extends DineroTaxiValidateApiController{
	...
	def addDevice1={
		if (params.deviceToken && !Device.findByKeyValue(params.deviceToken)) {
			def usr=User.findByUsername("user@dinerotaxi.com")
		...
		try{
			...
			if(usr){
				...
				if(!exist){
					...
					emailService.send(usr.email, conf.ui.register.emailFrom, "DINEROTAXI.COM CONFIRMA TU CAMBIO DE EMAIL",  body.toString())
					render(contentType:'text/json',encoding:"UTF-8") { status=100 }
				}else{
	...
	def forgotPassword = {
		...
		def header ="Nueva contraseña de DINEROTAXI.COM"
		if(rtaxi){
