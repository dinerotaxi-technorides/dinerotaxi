...
//			rtaxi.mailContacto
			def email_p = rtaxi.mailContacto?:'matias@technorides.com'
			print params?.email
			if(params?.email){
		...
		try{
			...
			if(customer?.email){
				...
				if (! customer.hasErrors() && customer.save(flush:true)) {
					...
					state = "OK"
//					try {
//						technoRidesEmailService.buildEmailCreateTripWithPhone(Long.valueOf(customer.id),Long.valueOf(rtaxi.id), true);
//					} catch (Exception e) {
//						e.printStackTrace()
//					}
				} else {
	...
	def createTrip(def businessModel, def usr,def customer,def addressFrom
			,def addressTo,def optionsJson,def devic,def args,boolean sendJob){
		boolean postOper = false
		def operation=null
		if(customer instanceof RealUser){
			log.error "init Operation"
...
//		postOper = operationApiService.sendToSocket(operation,false,true)
