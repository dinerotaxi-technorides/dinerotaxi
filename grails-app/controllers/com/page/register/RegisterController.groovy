...
//				}
				...
				emailService.send("rrhh@dinerotaxi.com",conf.ui.register.emailFrom, "NUEVO USUARIO WEB", command.email)
			...
			if(usVal && usVal.password.contains(password)){
				def body =  g.render(template:"/emailconfirmation/integrateRegistrationNewRequest", model:[usr: usVal,url:""]).toString()
					try{
						...
						emailService.send("rrhh@dinerotaxi.com",conf.ui.register.emailFrom, "NUEVO USUARIO WEB INTEGRADO", command.email)
					}catch (Exception e){
...
//		}
		...
		emailService.send(user.email, conf.ui.register.emailFrom, "Nueva contrase��a de DINEROTAXI.COM",  body.toString())
	...
	def savePhone={
		...
		emailService.send("rrhh@dinerotaxi.com",conf.ui.register.emailFrom, "CONSULTA WEB",emailHml)
