...
//				}
	...
	def getMetricsByUserCorporate(CorporateUser user, Company rtaxi){
	...
	def generateReport( params,rtaxi,CostCenter center) {
		...
		print sf.parse( params?.dateFrom)
		print sf.parse( params?.dateTo)
		...
		def customers = OperationCompanyHistory.createCriteria().list() {
			and{
				eq('company',rtaxi)
				between("createdDate", sf.parse( params?.dateFrom),sf.parse( params?.dateTo) )
		...
		for (oper in customers) {
			...
			def data = "${oper.id},${utilsApiService.generateFormatedAddressToClient(oper.createdDate,oper.user)},${firstName},${lastName},${driver},${oper.user.phone},${placeFrom},${placeTo},${oper.favorites.comments},${oper.stars},${oper.status.toString()},${oper.amount}"
			report.write(data+"\n")
		}
		report.flush();
		report.close();
